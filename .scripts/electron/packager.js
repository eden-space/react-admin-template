...
if (fs.existsSync(paths.appElectronConfigPath)) {
  ...
  rimraf(outputPath, (err) => {
    ...
    console.log(chalk.yellowBright(`  ${chalk.cyan('•')} 打包输出目录【${outputPath}】清理成功`));
    console.log(chalk.yellowBright(`  ${chalk.cyan('•')} 开始打包...`));
      ...
      .then((res) => {
        ...
        console.log(
          `  ${chalk.bold.greenBright('✔')} ${chalk.cyanBright(
            `打包完成！可在${chalk.greenBright(`【${outputPath}】`)}目录查看`,
          )}`,
        );
      })
      .catch((err) => {
  ...
  console.log(
    chalk.yellowBright(
      `  没找到electron相关配置文件【${chalk.redBright(
        'electron.config.js',
      )}】，请确保该文件存在并配置正确`,
    ),
  );
