...
const TabsBar: React.FC = () => {
    ...
    if (rect) {
      ...
      setMenus([
        { type: 'refresh', label: '刷新', shortKey: 'Alt+F', visible: getVisible('refresh', data) },
        {
          ...
          label: '重新加载',
          shortKey: 'Alt+D',
          visible: getVisible('deep-refresh', data),
        },
        ...
        { type: 'close', label: '关闭', shortKey: 'Alt+W', visible: getVisible('close', data) },
        {
          ...
          label: '关闭其他',
          shortKey: 'Alt+O',
          visible: getVisible('close-others', data),
        },
        {
          type: 'close-all',
          label: '关闭所有',
          shortKey: 'Alt+A',
          visible: getVisible('close-all', data),
        },
        {
          type: 'close-left',
          label: '关闭左侧',
          shortKey: 'Alt+L',
          visible: getVisible('close-left', data),
        },
        {
          type: 'close-right',
          label: '关闭右侧',
          shortKey: 'Alt+R',
          visible: getVisible('close-right', data),
        },
      ]);
    }
  }
  ...
  function getVisible(type: string, data: ITabItem) {
    ...
    const showCloseRight =
      (lastPinIndex === -1 && currentIndex < length - 1) ||
    ...
    switch (menu.type) {
      ...
      case 'close-others':
        list = tabsList
          ...
          .filter((item) => !comparePathname(item.path, path));
        break;
      case 'close-all':
    ...
    <div className={s.container}>
      ...
      >
        {menus
          ...
          .map(({ type, label, shortKey }) =>
            type === 'divider' ? (
              <Divider className={s.divider} key={type} />
            ) : (
              <li
                key={type}
